...
function handleSort() {
    ...
    globalData.sort((a, b) => {
        ...
        const aVal = (sortBy === 'invested') ? a.invested : (a.currentNav * a.units - a.invested);
        const bVal = (sortBy === 'invested') ? b.invested : (b.currentNav * b.units - b.invested);
        return bVal - aVal;
    });
    renderTable();
}
...
function renderTable() {
    ...
    let html = "";
    let gInv = 0, gCur = 0, gFlows = [];

    globalData.forEach(f => {
        const curVal = f.currentNav * f.units;
        const retRs = curVal - f.invested;
        const absPct = (retRs / f.invested) * 100;
        const xirr = calculateXIRR([...f.flows, { date: new Date(), amount: curVal }]);

        gInv += f.invested; gCur += curVal; gFlows.push(...f.flows);
        ...
        html += `
            <tr>
                ...
                <td class="${retRs >= 0 ? 'gain' : 'loss'}">₹${retRs.toLocaleString('en-IN', {maximumFractionDigits:0})}</td>
    ...
    document.getElementById('total-invested').innerText = "₹" + gInv.toLocaleString('en-IN');
    document.getElementById('total-current').innerText = "₹" + gCur.toLocaleString('en-IN');

    const totalXirr = calculateXIRR([...gFlows, { date: new Date(), amount: gCur }]);
    document.getElementById('total-xirr').innerText = totalXirr.toFixed(2) + "%";

    // Update Timestamp
    const now = new Date();
    const syncElem = document.getElementById('sync-time');
    if(syncElem) syncElem.innerText = now.toLocaleDateString() + " " + now.toLocaleTimeString();
...
async function submitData() {
    ...
    if (!data.date || !data.id || !data.units) { alert("Please fill all fields!"); return; }
